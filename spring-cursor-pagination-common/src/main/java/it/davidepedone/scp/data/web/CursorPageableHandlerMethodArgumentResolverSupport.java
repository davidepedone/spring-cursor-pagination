 ...
 * information from web requests and thus allows injecting {@link CursorPageable}
 * instances into controller methods. Request properties to be parsed can be configured.
 * Default configuration uses request parameters beginning with
...
public abstract class CursorPageableHandlerMethodArgumentResolverSupport {
	...
	/**
	 * Configures the {@link CursorPageable} to be used as fallback in case no
	 * {@link CursorPageableDefault} or {@link CursorPageableDefault} (the latter only
	 * supported in legacy mode) can be found at the method parameter to be resolved.
	 * <p>
	 * If you set this to {@literal Optional#empty()}, be aware that you controller
	 * methods will get {@literal null} handed into them in case no {@link CursorPageable}
	 * data can be found in the request. Note, that doing so will require you supply bot
	 * the page <em>and</em> the size parameter with the requests as there will be no
	 * default for any of the parameters available.
	...
	/**
	 * Configures a general prefix to be prepended to the page number and page size
	 * parameters. Useful to namespace the property names used in case they are clashing
	 * with ones used by your application. By default, no prefix is used.
	 ...
	 * properties. Defaults to {@code _}. So a qualifier of {@code foo} will result in a
	 * page number parameter of {@code foo_page}.
	 * @param qualifierDelimiter the delimiter to be used or {@literal null} to reset to
	 * the default.
	 */
	...
	/**
	 * Returns the name of the request parameter to find the {@link Pageable} information
	 * in. Inspects the given {@link MethodParameter} for {@link Qualifier} present and
	 * prefixes the given source parameter name with it.
	...
	protected CursorPageable getPageable(MethodParameter methodParameter, @Nullable String pageSizeString,
		...
		if (!pageSize.isPresent() && !defaultOrFallback.isPresent()) {
	...
	/**
	 * Tries to parse the given {@link String} into an integer and applies the given
	 * boundaries. Will return 0 if the {@link String} cannot be parsed.
	...
	/**
	 * Retrieves the maximum page size to be accepted. This allows to put an upper
	 * boundary of the page size to prevent potential attacks trying to issue an
	 * {@link OutOfMemoryError}. Defaults to {@link #DEFAULT_MAX_PAGE_SIZE}.
	...
	/**
	 * Asserts that every {@link CursorPageable} parameter of the given parameters carries
	 * an {@link Qualifier} annotation to distinguish them from each other.
